...
int main(void) {
    ...
    pid_sort_r = fork();
    if (pid_sort_r < 0) {
        /* parent exits if sort r child fails */
    ...
    if (pid_sort_r == 0) {
        ...
        /* create if needed, open write-only, truncate existing file; mode rw-rw-rw- */
        if (outfile_fd < 0) {
        ...
        execlp("sort", "sort", "-r", NULL); /* note: sort will keep reading until EOF */
    ...
    if (WEXITSTATUS(ls_child_status) != 0) {
        fprintf(stderr, "ls child exited with status %d\n", WEXITSTATUS(ls_child_status));
        exit(1);
    }
    ...
    if (WEXITSTATUS(sort_r_child_status) != 0) {
        fprintf(stderr, "sort -r child exited with status %d\n", WEXITSTATUS(sort_r_child_status));
        exit(1);
    }
